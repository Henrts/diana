...
```javascript
  CloseableChip.extendStyles(() => ({chip:{backgroundColor:"grey",borderColor:"yellow"}}), {register:true});
<ChipList list={["test", "test2", "test3"]} />
```
...
```javascript
CloseableChip.extendStyles(() => ({ chip: { backgroundColor: "steelblue", borderColor: "red" } }),{ register: true });
<ChipInput value={[]} />;
```
